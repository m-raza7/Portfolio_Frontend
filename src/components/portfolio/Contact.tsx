...
import { Mail, Github, Linkedin, Twitter, Download, Send, Loader2 } from "lucide-react";
...
function validate(data: FormState): Errors {
  ...
  if (!data.subject.trim() || data.subject.trim().length < 2) e.subject = "Subject is required";
    ...
    try {
      ...
      if (data?.errors) {
        ...
        Object.entries(data.errors).forEach(([k, v]) => {
          fieldErrors[k as keyof FormState] = Array.isArray(v) ? (v[0] as string) : String(v);
        });
    ...
    >
      <div className="grid gap-8 lg:grid-cols-[1fr_1.2fr]">
        ...
        >
          ...
          <p className="mt-2 text-sm text-muted-foreground">I usually reply within 24 hours.</p>
          <a
        ...
        >
          <div className="grid gap-4 md:grid-cols-2">
            <Field label="Your name" value={form.name} onChange={onChange("name")} error={errors.name} placeholder="Ada Lovelace" />
            <Field label="Email" type="email" value={form.email} onChange={onChange("email")} error={errors.email} placeholder="you@domain.com" />
          </div>
          <Field label="Subject" value={form.subject} onChange={onChange("subject")} error={errors.subject} placeholder="Project / role / collab" />
          <div>
            ...
            <textarea
              ...
              className={`w-full resize-none rounded-2xl border bg-white/5 px-4 py-3 text-sm outline-none transition focus:bg-white/10 ${
                errors.message ? "border-red-400 focus:border-red-400" : "border-white/10 focus:border-[var(--neon-pink)]"
              }`}
            />
    ...
    <div>
      ...
      <input
        ...
        className={`w-full rounded-2xl border bg-white/5 px-4 py-3 text-sm outline-none transition focus:bg-white/10 ${
          error ? "border-red-400 focus:border-red-400" : "border-white/10 focus:border-[var(--neon-pink)]"
        }`}
      />
